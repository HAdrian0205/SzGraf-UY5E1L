...
void bounce_ball(Pong* pong)
{
    if (pong->ball.x - pong->ball.radius < 50) {
        if(pong->ball.y >= pong->left_pad.y && pong->ball.y <= (pong->left_pad.y + pong->left_pad.height) + pong->ball.radius) {
            pong->ball.x = pong->ball.radius + 50;
            pong->ball.speed_x *= -1;
            pong->ball.rotation = 40;
        } else {
    ...
    if (pong->ball.x + pong->ball.radius > pong->width - 50) {
        if(pong->ball.y >= pong->right_pad.y && pong->ball.y <= (pong->right_pad.y + pong->right_pad.height) + pong->ball.radius) {
