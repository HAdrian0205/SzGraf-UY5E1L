    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            if(event.button.button == SDL_BUTTON_LEFT) {
                ...
                game->pong.ball.x = mouse_x - 25;
                game->pong.ball.y = mouse_y - 25;
            }
            break;
        case SDL_QUIT:
            game->is_running = false;
...
void update_game(Game* game)
{
    ...
    sprintf(title, "Bal: %d | Jobb: %d", game->pong.left_points, game->pong.right_points);
